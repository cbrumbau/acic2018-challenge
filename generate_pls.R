#!/usr/bin/env Rscript
#
# Rscript generate_pls.R --folds --include --merge imputations_folder output_folder
...
pls <-function(dataset.name, imputation.list) {
	...
	start.time <- Sys.time()
	model.list <- foreach(this.x=imputed.x, this.y=imputed.y, .inorder=TRUE, .packages='pls') %dopar% {
		plsr(this.y ~ ., data=this.x, ncomp=ncol(this.x), validation="CV", segments=opt$options$folds[1])
	}
